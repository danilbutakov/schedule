...
echo "Запускаем скрипт полного исправления проекта..."
...
# Создаем новый файл проекта Xcode без пользовательских скриптов
echo "Создаем новый файл проекта Xcode без пользовательских скриптов"
...
# Создаем файл для отключения expo-dev-menu
echo "Создаем файл для отключения expo-dev-menu"
cat > ios/DisableDevMenu.swift << EOL
import Foundation

#if DISABLE_EXPO_DEV_MENU
// Этот файл нужен для отключения expo-dev-menu в режиме сборки
// Он используется для исправления ошибок компиляции с expo-dev-menu-interface
#endif
EOL

# Очищаем кэш CocoaPods
echo "Очищаем кэш CocoaPods"
rm -rf ios/Pods
rm -rf ios/Podfile.lock
rm -rf ios/build

# Устанавливаем поды
echo "Устанавливаем поды"
cd ios && pod install && cd ..

echo "Скрипт полного исправления проекта завершен" 